...
def main():
    ...
    if args.report_only:
        from evaluation.results import report
        report()
        return
    ...
    print(f"cell lines: {cfg.CELL_LINES}")
    ...
    from evaluation.results import beats_baseline, ranking, report
    from evaluation.run import run

    out_name = "metrics_multidose" if args.multi_dose else "metrics"
    run(present, models, force=args.force, out_name=out_name,
        multi_dose=args.multi_dose)
    report(load_results(out_name))
    print("\nranking (ridge)")
...
def parse_args():
    ...
    p.add_argument("--report-only", action="store_true", help="print the report and exit")
    p.add_argument("--multi-dose", action="store_true",
                   help="stack all four doses with log-dose and the interaction term")
    return p.parse_args()
